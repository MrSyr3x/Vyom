        ...
        if let Ok(mut buf) = buffer.lock() {
            ...
            if channels == 2 {
                for chunk in new_samples.chunks(2) {
                    if chunk.len() == 2 { buf.push_back((chunk[0] + chunk[1]) / 2.0); }
                }
            } else {
                for &s in new_samples { buf.push_back(s); }
            }
    ...
    pub fn get_bars(&mut self, count: usize) -> Vec<f32> {
        let samples = {
            if let Ok(buf) = self.audio_buffer.lock() {
                if buf.len() < self.fft_size { return vec![0.0; count]; }
                buf.iter().rev().take(self.fft_size).cloned().collect::<Vec<_>>()
            } else { return vec![0.0; count]; }
        };
        ...
        if raw_data.is_empty() { return bars; }
        ...
        // AGC Decay
        self.max_val *= 0.995; 
        if self.max_val < 0.1 { self.max_val = 0.1; } 
        ...
        for i in 0..count {
            ...
            for (fr, val) in raw_data.iter() {
                let f = fr.val();
                if f < range_min { continue; }
                if f > range_max { break; }
            ...
            let avg_val = if total_weight > 0.0001 { weighted_sum / total_weight } else { 0.0 };
            ...
            if corrected_val > current_frame_max { current_frame_max = corrected_val; }
        ...
        // Normalize first
        for i in 0..count {
            let norm = if self.max_val > 0.0 { bars[i] / self.max_val } else { 0.0 };
        ...
        for i in 1..count {
            if bars[i] < bars[i-1] * monster_decay { bars[i] = bars[i-1] * monster_decay; }
        }
        ...
        if self.prev_bars.len() != count { self.prev_bars = vec![0.0; count]; }
        if self.velocities.len() != count { self.velocities = vec![0.0; count]; }
        ...
        for i in 0..count {
            ...
            // Silence Gate
            if input_val < 0.005 { input_val = 0.0; }
            ...
            if input_val > display_val {
                 ...
                 if display_val < 0.0 { display_val = 0.0; vel = 0.0; }
            }
